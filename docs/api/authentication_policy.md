...
During the interactions with the Arrowhead Core and Support systems, all client are required to prove their identity by providing appropriate identity info. The authentication can be done based on the given identity info.
...
**The interactions between the consumers and providers should never contain identitiy tokens in order to prevent any kind of identitiy theft. Identities should only be shared with the trusted Arrowhead Core and Support systems.**
...
The [declared](#declared) indentiy info is the unique system name itself that must be present in the [HTTP Authorization request header](https://datatracker.ietf.org/doc/html/rfc7235#section-4.2) according to the following scheme: 

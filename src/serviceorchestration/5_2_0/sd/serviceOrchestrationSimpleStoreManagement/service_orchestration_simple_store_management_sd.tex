...
\ArrowheadTitle{serviceOrchestrationSimpleStoreManagement} % XXX = ServiceName 
\ArrowheadServiceID{serviceOrchestrationSimpleStoreManagement} % ID name of service
...
\begin{center}
  ...
  % Front Page Abstract
  \begin{abstract}
    This document provides service description for the \textbf{serviceOrchestrationSimpleStoreManagement} service. 
  \end{abstract}
...
\section{Overview}
\label{sec:overview}
This document describes the \textbf{serviceOrchestrationSimpleStoreManagement} service, which enables systems with administrative rights to manage the peer-to-peer orchestration rules within an Eclipse Arrowhead Local Cloud (LC). These rules associate consumer systems to service instances and can be used during orchestration process in case of simple-store strategy.


The \textbf{serviceOrchestrationSimpleStoreManagement} service contains the following operations:
...
This section describes the abstract signatures of each operations of the service. The \textbf{serviceOrchestrationSimpleStoreManagement} service is used to perform \textit{query, create, modify-priorities} and \textit{remove} operations on orchestration rules.
...
Operation \textit{query} lists the orchestration store records that match the filtering requirements. The query data must meet the following criteria:
...
\fsubsection{modify-priorities}{OrchestrationSimpleStorePriorityRequest}{OrchestrationSimpleStoreListResponse}{}{ErrorResponse}
...
Operation \textit{remove} requires the rule identifiers and removes their associated orchestration rule records.
...
Here, all data objects that can be part of the \textbf{serviceOrchestrationSimpleStoreManagement} service are listed and must be respected by the hosting System.
...
\begin{table}[ht!]
\begin{tabularx}{\textwidth}{| p{3cm} | p{3.8cm} | p{1.75cm} | X |} \hline

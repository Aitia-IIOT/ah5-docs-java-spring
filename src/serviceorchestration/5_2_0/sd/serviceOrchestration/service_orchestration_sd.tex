...
qualityRequirements & \pref{List}$<$\hyperref[sec:model:QoSRequirement]{QoSRequirement}$>$ & no & Service quality requirements in order of priority. \\ \hline
exclusivityDuration & \pref{Number} & no & The interval the service wanted to be exclusive.  \\ \hline
\end{tabularx}
\end{table}

\textbf{\textit{Note}}: \textit{simple-store} strategy supports only to provide the \texttt{serviceRequirement} and \texttt{orchestrationFlags} fields. If other fields are specified, their value will not be considered and the ignored fields will appear in a warning message.
...
\textbf{\textit{Note}}: \textit{simple-store} strategy supports only to provide the \texttt{serviceDefinition} and \texttt{preferredProviders} fields. If other fields are specified, their value will not be considered and the ignored fields will appear in a warning message.
...
\textbf{\textit{Note}}: in case of \textit{simple-store} strategy, not supported flags will not be considered and the ignored flags will appear in a warning message.

\msubsection{struct}{QoSRequirement}

\begin{table}[ht!]
\begin{tabularx}{\textwidth}{| p{3.25cm} | p{3.8cm} | p{2cm} | X |} \hline
\rowcolor{gray!33} Field & Type & Mandatory & Description \\ \hline
type & \pref{QoSEvaluationType} & yes & Specifies an exact evaluation method. \\ \hline
operation & \pref{QoSOperation} & yes & Specifies an operation to handle the QoS evaluation results. \\ \hline
requirements & \hyperref[sec:model:Metadata]{Metadata} & no & Specifies the QoS evaluation method specific properties. \\ \hline
\end{tabularx}
\end{table}
...
\textbf{\textit{Note}}: \textit{simple-store} strategy supports only to provide the \texttt{serviceInctanceId, providerName, \allowbreak serviceDefinition, cloudIdentifier} and \texttt{version} fields.
...
\pdef{QoSEvaluationType} & A string name of a specific QoS evaluation method. \\ \hline
\pdef{QoSOperation}     & A string name that specifies how to handle QoS evaluation results (filter, sort, etc.). \\ \hline

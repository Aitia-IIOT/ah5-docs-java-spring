...
\begin{itemize}
    ...
    \item An orchestration result does not contain enough information to consume a service instance. The consumer at least needs to contact the ServiceRegistry Core system using the service instance id to acquire the access information of the referenced service instance. If this instance requires authorization token as a security measure, the consumer also has to consult with the ConsumerAuthorization Core system to acquire one.
\end{itemize} 
...
For authentication, the SimpleStoreServiceOrchestration can utilize another Core system, the Authentication system's service to verify the identities of the requester systems.  If no Authentication system is deployed into the Local Cloud, the SimpleStoreServiceOrchestration can accept X.509 certificates for identification. If none of these are available, the SimpleStoreServiceOrchestration trusts the requester system’s self-provided identity.

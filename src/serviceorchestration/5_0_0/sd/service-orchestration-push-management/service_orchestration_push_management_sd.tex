...
\begin{itemize}
    ...
    \item \textit{trigger} initiates service orchestration processes based on consumer system names or subscription identifiers or a creator system name;
    \item \textit{query} lists the existing subscriptions that match the filtering requirements
\end{itemize}
...
In certain use cases, the right of deciding when and how the service orchestration process should be executed does not belong to the consumer system itself, but is assigned to a higher level entity (a dedicated system or a human operator). This higher entity invokes the \textit{trigger} operation when it deems that a re-orchestration is necessary.
...
metadataRequirements & \pref{List}$<$\hyperref[sec:model:MetadataRequirements]{MetadataRequirements}$>$ & no & The orchestrated service must meet at least to one of the specified metadata requirements. \\ \hline
...
Specific \pdef{String}:\pdef{Boolean} pair to control the orchestration process. Possible keys:

...
\section{Overview}
\label{sec:overview}
This document describes the \textbf{blacklistManagement} service, which enables systems (with operator role or proper permissions) to handle (query, create, remove) blacklist entries in bulk.
...
\rowcolor{gray!33} Field & Type & Mandatory & Description \\ \hline
authentication & \hyperref[sec:model:Identity]{Identity} & yes & The requester of the operation. \\ \hline
pagination & \hyperref[sec:model:PageRequest]{PageRequest} & no & Paging information about the queried blacklist entries. \\ \hline

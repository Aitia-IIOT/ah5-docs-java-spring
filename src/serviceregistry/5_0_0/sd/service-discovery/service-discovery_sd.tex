...
\ArrowheadTitle{service-discovery} % XXX = ServiceName 
\ArrowheadServiceID{service-discovery} % ID name of service
...
\begin{center}
  ...
  % Front Page Abstract
  \begin{abstract}
    This document provides service description for the \textbf{service-discovery} service. 
  \end{abstract}
...
\begin{itemize}
    ...
    \item \textit{lookup} lists the service instances that matches the filtering requirements;
\end{itemize}
...
However, there are some special cases when looking for a service instance can be requested anonymously.  For example, when somebody looks for a service instance that has to consume to get an identity (some kind of login).
...
\begin{itemize}
    ...
    \item Service definition names can contain maximum 63 character of letters (english alphabet), numbers and dash (-), and has to start with a letter (also cannot ends with dash).
...
createdAt & \pref{DateTime} & Service instance is registered at this timestamp. \\ \hline
updatedAt & \pref{DateTime} & Service instance is modified at this timestamp. \\ \hline
\end{tabularx}
\end{table}
...
createdAt & \pref{DateTime} & Service definition is registered at this timestamp. \\ \hline
updatedAt & \pref{DateTime} & Service definition is modified at this timestamp. \\ \hline
\end{tabularx}
\end{table}
...
metadataRequirementsList & \pref{List}$<$\hyperref[sec:model:MetadataRequirements]{MetadataRequirements}$>$ & no & Requester is looking for service ins\-tances that are matching the specified metadata requirements.  \\ \hline
...
interfacePropertyRequirementsList & \pref{List}$<$\hyperref[sec:model:MetadataRequirements]{MetadataRequirements}$>$ & no & Requester is looking for service ins\-tances with interfaces that are matching the specified properties requirements.  \\ \hline
\end{tabularx}
\end{table}

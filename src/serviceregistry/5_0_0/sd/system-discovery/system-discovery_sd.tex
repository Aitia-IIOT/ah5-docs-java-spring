...
\begin{itemize}
    ...
    \item \textit{lookup} shows the systems that matches the filtering requirements;
\end{itemize}
...
\begin{itemize}
    ...
    \item Systems can register multiple times, but only if there are identical (same version, addresses, metadata and device).
\end{itemize}
...
Here, all data objects that can be part of the \textbf{system-discovery} service provides to the hosting System are listed.
